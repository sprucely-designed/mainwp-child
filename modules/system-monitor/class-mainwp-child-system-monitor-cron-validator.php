<?php
/**
 * MainWP Child System Monitor Cron Validator.
 *
 * @since 6.1.4
 *
 ...
 */
class MainWP_Child_System_Monitor_Cron_Validator {
        ...
        if ( 0 === $last_cron_run ) {
            ...
            if ( $next_run > 0 && time() > ( $next_run + self::FIRST_RUN_GRACE_PERIOD ) ) {
                ...
                $issues[] = array(
                    ...
                    'severity'   => MainWP_Child_System_Monitor_Issues::SEVERITY_WARNING,
                    'data'       => array(
                        'delay'            => time() - $next_run,
    ...
    private function get_monitor_stale_severity( $delay ) {
        ...
        if ( $delay > self::STALE_ERROR ) {
            return MainWP_Child_System_Monitor_Issues::SEVERITY_ERROR;
        }

        if ( $delay > self::STALE_WARNING ) {
            return MainWP_Child_System_Monitor_Issues::SEVERITY_WARNING;
        }
